...
class InstructFollowEval(BaseModelEval):
    """
    评估基类：
    ...
    def main_eval(self, model: str, url: str, eval_column_list: list[str], df, output_dir: str, prompt_path: str, thread_num: int, chunk_num: int, temperature: float):
        ...
        # 读取待评估文件并与prompt进行拼接
        # 如果是gpt4
        # 由于将拼接好的prompt存入了prompts列，因此取query_column_name = "prompts"
        if(model == "gpt4" or model == "wenxin"):
            responses = get_gpt4api_df(df_with_prompts, query_column_name = "prompts", message_type='', 
                                       max_request_times = 5, qps=10, max_concurrent=20, asyncio_flag=True, url=url)
        # 其余模型
        else:
        ...
        #     self.logger.error("error while instruct follow eval:{}".format(e))
        return rel_result,rel_reason
        
instructFollowEval = InstructFollowEval("instruct_follow_eval")