...
class VllmConfig:
    def __init__(self, model, url, temperature=0.9, top_p=0.9, max_tokens=2048, chunk_num=4, thread_num=10, query_column_name='prompts', response_column_name='response'):
...
class CallLLMByVllm(object):
    ...
    def task(self, row: dict):
        ...
        response = self.complete_response(
            messages=data, client=self.config.client, stream=False, **self.config.generate_config)
