...
class CorrectFilter(BaseModelEval):
    """
    评估基类：
    ...
    def result_parse(self, response):
        ...
        try:
            ...
            relevance_match = re.search(r"相关性得分: \{\{(.*?)\}\}", response, re.S)
            authenticity_match = re.search(r"真实性得分: \{\{(.*?)\}\}", response, re.S)
            accuracy_match = re.search(r"准确性得分: \{\{(.*?)\}\}", response, re.S)
            
            if relevance_match:
                result_dict["相关性"] = float(relevance_match.group(1))
            if authenticity_match:
                result_dict["真实性"] = float(authenticity_match.group(1))
            if accuracy_match:
                result_dict["准确性"] = float(accuracy_match.group(1))
        
        except Exception as e:
            print(f"[rel-eval]:结果解析失败！错误信息: {e}")
        
        return result_dict["准确性"]
...
correctFilter = CorrectFilter("correct_ans_filter")