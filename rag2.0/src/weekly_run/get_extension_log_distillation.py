...
class GPT4DistillationExtended(BaseDistillation):
    ...
    def split_you_time_command_queries(self, query_ls: list):
        ...
        for query in query_ls:
            ...
            # 检查时效性
            if any(time_word in query['query'] for time_word in time_word_list):
                bad_query_dict['tag'] = '时效性'
            # 检查人设
            elif '你' in query['query']:
                bad_query_dict['tag'] = '人设你'
            # 检查任务型
            elif re.search(command_pattern, query['query']):
    ...
    def get_unique_query(self, query_ls, batch_size=32):
        ...
        # 提取 query 文本
        query_texts = [q['query'] for q in query_ls]
    ...
    def get_distillation_data(self, date):
        ...
        query_ls_with_metadata = self.get_new_query_ls_with_metadata(df)
        ...
        # 将 unique_queries_with_metadata 转换为 DataFrame，并保留所有列
        unique_queries_df = pd.DataFrame(unique_queries_with_metadata).rename(columns={'query':'user-query'})
        unique_queries_df.to_csv(os.path.join(outfolder, f'{date}_unique_queries.csv'), index=False)

        # 生成api thought obs gpt4
        df_api = get_api_df(unique_queries_df, col_query='user-query', output_file=os.path.join(outfolder, f'{date}_api.csv'), url=self.config['api_url'])
