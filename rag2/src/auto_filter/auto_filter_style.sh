...
# 获取 model_list_str 的第一个元素
model_list_first=$(echo $model_list_str | cut -d',' -f1)
url_list_first=$(echo $url_list_str | cut -d',' -f1)
# 将第一个元素转换为长度为1的列表
model_list_str="$model_list_first"
url_list_str="$url_list_first"
...
python multi_filter_assistant.py  \
    ...
    --concat_prompt_flag #默认进入代码后拼接prompt，若提前拼好，此处可设置为 --no_concat_prompt_flag; 提前拼好后默认取eval_column_list[0]为eval_col，会把eval_col改成llm_prompts
