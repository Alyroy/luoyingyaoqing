...
def shuffle_obs(obs):
    try:
        ...
        # 随机选择方法: "random", "original" 或 "reverse"
        method = random.choice(["random", "original", "reverse"])
...
#         # 如果文件不存在或为空，则写入表头
#         if not file_exists:
#             writer.writerow(["user-query","thought", "api", "observation", "raw_observation"])  # 写入标题行
...
#                 writer.writerow([query, thought, api, new_obs, multi_llm_obs]) 
#             except Exception as e:
