...
WORK_DIR=/mnt/pfs-guan-ssai/nlu/renhuimin/rag_tool/src/tool_kg_search/1b_model_deployment/  # 工作目录需要修改

cd $WORK_DIR
pip3 install -r $WORK_DIR/requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple

set -xv
export CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7"

SERVER_NAME=$(hostname -i)  # b区服务器的主机结点
BROAD_CAST_FILE=/mnt/pfs-guan-ssai/nlu/gongwuxuan/public/api_url.log
# 检查并创建目录
