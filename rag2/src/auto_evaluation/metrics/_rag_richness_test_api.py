...
class RichnessTestAPIEval(BaseModelEval):
    ...
    def result_parse(self,response, col_score='pred_score', col_reason='pred_reason'):
        """
        解析相关性打分
        """
        try:
            ...
            ret = ele.get(col_score, -1)
            reson = ele.get(col_reason, "<|wrong parse|>")
            return ret, reson
        except Exception as exc:
            traceback.print_exc()
            return -1, "<|wrong parse|>"
    ...
    def main_eval(self, model: str, url: str, eval_column_list: list[str], df, output_dir: str, prompt_path: str, thread_num: int, chunk_num: int, temperature: float, eval_mode:str = 'user_obs_ans_concat', max_tokens=8000, input_text_type='default', eval_task='richness_eval_v1'):
        '''
        主评估函数
        '''
        try:
            ...
            # 其余模型
                ...
                config = TestAPIConfig(
                    ...
                    test_api='http://172.24.139.92:30834/predict'
                )
            ...
            if eval_task.startswith('richness'):
                col_score = 'pred_rich_score'
                col_reason = 'pred_rich_reason'
            elif eval_task == 'repeat_eval':
                col_score = 'pred_repeat_score'
                col_reason = 'pred_repeat_reason'
                
            rich_result = []
            rich_reson = []
            for resp in response_sorted_list:
                rich_score, rich_reason = self.result_parse(resp, col_score, col_reason) # 解析模型回复
                rich_result.append(rich_score)
                rich_reson.append(rich_reason)
            ...
            rich_reson_expand = []
            for i in range(len(df)):
                if i < len(rich_result):
                    pass
                else:
                    rich_result_expand.append(-1)
                    rich_reson_expand.append("<|wrong parse|>")

            if rich_result_expand:
                rich_result.extend(rich_result_expand)
            
            if rich_reson_expand:
                rich_reson.extend(rich_reson_expand)
                
            print("error while richness eval:{}".format(e))
            traceback.print_exc()
            # self.logger.error("error while relevance eval:{}".format(e))

        return rich_result, rich_reson

richnessTestAPIEval = RichnessTestAPIEval("richness_test_api_eval")
