...
python vllm_qwen2swa_inference.py --input_file ${INPUT_DIR} --output_path ${OUTPUT_DIR} --model ${EVAL_MODEL} --tiktoken_path ${tiktoken_model_path} --time_stamp ${EVAL_TIMESTAMP} --batch_size 2 --turn_mode qwen --eval_col ${EVAL_COL} --temperature ${temperature} --top_p ${top_p} --repetition ${repetition}
