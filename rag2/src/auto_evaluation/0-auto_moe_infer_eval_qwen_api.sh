...
declare -a MODEL_CKPT_DIR_LIST=(
    "/mnt/pfs-guan-ssai/nlu/renhuimin/checkpoints/16b_sft_mindgpt_20240911_norag_140w_v7moe_32k_model_1/checkpoint-4299"
    # 可以在这里添加更多的模型路径
)

declare -a QUEUE_NAME_LIST=("wq-app")  # 根据需要添加更多队列名
declare -a JOB_NAME_LIST=("ragmoe0927yuqing1")  # 根据需要添加更多任务名
...
for i in "${!MODEL_CKPT_DIR_LIST[@]}"; do
    ...
    (
        # 0. 判断模型是否训练完成
        cnt=1
        echo "${MODEL_CKPT_DIR}/successed"
        until [ -f "${MODEL_CKPT_DIR}/successed" ]; do
            echo "等待 ${MODEL_CKPT_DIR} 模型训练完毕...已等待 ${cnt} 小时"
            sleep 3600
            cnt=$(expr $cnt + 1)
        done
        
        # 判断是否需要transfer hf model
        ...
        echo sh auto_lizrun_guan_livis_moe.sh ${CURRENT_DIR} ${JOB_NAME} ${QUEUE_NAME} 1 ${HF_MODEL_CKPT_DIR} ${JOB_NAME} ${INPUT_DIR} ${OUTPUT_DIR} ${EVAL_COL}
        sh auto_lizrun_guan_livis_moe.sh ${CURRENT_DIR} ${JOB_NAME} ${QUEUE_NAME} 1 ${HF_MODEL_CKPT_DIR} ${JOB_NAME} ${INPUT_DIR} ${OUTPUT_DIR} ${EVAL_COL}
    
        # 2. 判断推理是否完成
        cnt=1
        echo "${OUTPUT_DIR}/${JOB_NAME}/.done"
