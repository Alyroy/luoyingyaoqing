...
################################################
# 全局参数设置
CURRENT_DIR=/mnt/pfs-guan-ssai/nlu/jiajuntong/code/auto_evaluation
cd $CURRENT_DIR
...
declare -a QUEUE_NAME_LIST=("wq-sft")
declare -a JOB_NAME_LIST=("test-qwen")
# USER_NAME=jiajuntong

# 输入与输出目录设定
INPUT_DIR=${CURRENT_DIR}'/eval_data/safety_0115' # query扩展形式  livis_query_fc_1114/select-new-1231-lvfc_form-2k_final_input.csv1113.out.csv 旧测试集
OUTPUT_DIR=${CURRENT_DIR}'/eval_results/'
EVAL_COL=model_13b_input # 无需替换
INPUT_PREDICT_COL=predict_output # 无需替换
# 指标统计
file=testset687_e2e_wo_safety.csv
