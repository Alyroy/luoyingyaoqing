...
def do_func_api_single(gpu_no, params, input_f, api_flag=True, bsize=20, loop=5, mode="moe", checkpoint='', directory='', total_gpus=[0], tiktoken_model_path=None):
    ...
    for one_file in file_list:
        ...
        if "汽车问答" not in one_file and "用车助手" not in one_file:
            sampling_params = SamplingParams(
                temperature=params.temperature, 
                top_p=params.top_p, 
                top_k=params.top_k,
        ...
        pd.DataFrame(df).to_csv(output_pt + ".csv", index=None, encoding='utf-8-sig')
...
def func_api_single(params, input_f, api_flag=True, bsize=20, loop=5, mode="moe", tiktoken_path=None):
    ...
    n_process = 1
    # do_func_api_single(0, params, input_f, api_flag, bsize, loop, mode, args.model, directory, args.gpus.split(','), tiktoken_path)
