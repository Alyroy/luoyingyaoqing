...
def extract_datetime_from_input(input_16b):
    ...
    if match:
        ...
        if user_date and user_time:
            # Convert the Chinese date and time format to standard format
            date_pattern = r"(\d+)年(\d+)月(\d+)日"
            time_pattern = r"(\d+)时(\d+)分"
            date_match = re.match(date_pattern, user_date)
            time_match = re.match(time_pattern, user_time)
...
class DataFilter:    
    ...
    def get_task_usecols(filter_df):
        ...
        desired_cols = ['user-query', 'assistant', 'api', 'thought', 'observation', 'system', 'system_without_time', 'context',
                        'generalized_question_from_user', 'generalized_question_from_assistant',
                        'task_name','uid','datetime']
    
        available_cols = filter_df.columns.tolist()
        cols_to_load = [col for col in desired_cols if col in available_cols]
        filter_df = filter_df[cols_to_load].rename(columns={'system':'system_with_time','system_without_time':'system'})
    ...
    def filter_bad_df(df, config):
        ...
        if config['is_rag_flag']:
            and_conditions.append("(is_rag == 1)")
        else:
            and_conditions.append("(is_rag == 0)")

        if config['is_single_flag']:
            and_conditions.append("(is_single_turn == 1)")
        else:
...
class PromptConstructor:
    ...
    def construct_prompt(row, oneshot_prompt, config):
        if config['is_single_flag'] and config['is_rag_flag']:
            ...
            ---
            下面是给出的实际问题：
            用户提问的日期是：{row['datetime']}
            问题：{row['user-query']}
            检索资料:{row['observation']}
            你的回答：
            """
