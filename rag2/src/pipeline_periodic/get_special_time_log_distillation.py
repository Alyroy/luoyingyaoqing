...
class GPT4TimeDistillation(BaseDistillation):
    ...
    def get_distillation_data(self, input_file):
        ...
        try:
            ...
            df = df[(df['is_realtime']==1)|(df['query_has_relative_time']==1)]
            ...
            filter_df = filter_df.drop(columns=['system'])
            filter_df = filter_df.rename(columns={'system_with_time':'system'})
            filter_df = filter_df.groupby('user-query').first().reset_index()
        except Exception as exc:
            print(f"Exception occurred while processing file: {input_file}")
            traceback.print_exc()
    
        df.to_csv(os.path.join(outfolder, f'{date}_raw_time_log_data.csv'), index=False, encoding='utf-8-sig')
