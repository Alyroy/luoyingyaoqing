...
# 参数组合
car_app_feishu_types=("log_distillation" "log_distillation_car") # "log_distillation_car"
    ...
    for car_app_feishu_type in "${car_app_feishu_types[@]}"; do
        for raw_extension_type in "${raw_extension_types[@]}"; do
            for atomic_type in "${atomic_types[@]}"; do
                # 检查依赖并执行任务，出错继续
                CheckDependencyFunction "$car_app_feishu_type" "$raw_extension_type" "$atomic_type" "$date" # || continue
                ExecuteJobFunction "$car_app_feishu_type" "$raw_extension_type" "$atomic_type" "$date" # || continue
            done
        done
    done
done