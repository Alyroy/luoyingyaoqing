...
JOB_NAME="filter_log_multi_rag"

current_date=$(date "+%Y-%m-%d")
date=$(date -d "${current_date} -3 day" "+%Y-%m-%d")
# date="2024-08-14"
...
CheckDependencyFunction() {
    ...
    local done_file="/mnt/pfs-guan-ssai/nlu/renhuimin/data/log_distillation/${date}/${raw_extension_type}/single_False_rag_True/gpt4_data/.done"
    if [ ! -f $done_file ]; then
...
ExecuteJobFunction() {
    ...
    folder="/mnt/pfs-guan-ssai/nlu/renhuimin/data/log_distillation/${date}/${raw_extension_type}/single_False_rag_True/"
...
# 调用函数并传递参数
CheckDependencyFunction "raw"
ExecuteJobFunction "raw"
CheckDoneFunction "raw"

# 调用函数并传递参数
CheckDependencyFunction "extension"
ExecuteJobFunction "extension"
CheckDoneFunction "extension"