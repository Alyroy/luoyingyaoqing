...
JOB_NAME="distillate_extension_log_multi_rag_car"
...
ExecuteJobFunction() {
    ...
    prompt_path="/mnt/pfs-guan-ssai/nlu/renhuimin/rag_tool/conf/generation_prompts/generation_多轮日志蒸馏.txt"
    ...
    # 对日志数据回复糟糕的部分进行蒸馏
    python get_extension_log_distillation.py --no_is_single_flag --is_rag_flag --model_name 'gpt4o' --model_url ${model_url} --base_output_path ${base_output_path} --prompt_path ${prompt_path} --api_url $api_url --input_file ${input_file}
